...
namespace WFP.ICT.Web.Helpers
{
    public class ExcelHelper
    {
            ...
            try
            {
                ...
                using (MemoryStream stream = new MemoryStream())
                {
                    ...
                    using (SpreadsheetDocument spreadSheet = SpreadsheetDocument.Open(stream, true))
                    {
                        ...
                        cell.CellValue = new CellValue(model.CTRPercentage);
                        cell.DataType = new EnumValue<CellValues>(CellValues.String);

                        // bounce & opt
                        cell = GetCell(worksheetPart.Worksheet, "A", 29);
                        cell.CellValue = new CellValue(new Random().Next(1,30).ToString());
                        cell.DataType = new EnumValue<CellValues>(CellValues.String);
                        ...
                        {
                            PopulateRow(worksheetPart.Worksheet, vm, start);
                            start++;
                        }
                        #endregion
        ...
        public static void PopulateRow(Worksheet worksheet, CampaignReportDetailVM row, uint rowNumber)
        {
