...
namespace ADSDataDirect.Web.HasOffers.Campaigns
{
    public class CampaignsManager : HasOffersManager
    {
        public GetCampaignsListResponse List()
        {
            using (HttpClient client = new HttpClient())
            {
                client.Timeout = TimeSpan.FromMinutes(1);
                string requestUri = $"{baseURL}/campaign/list?token={token}&limit=10&offset=10";
                ...
                using (HttpContent content = response.Content)
                {
                    ...
                    GetCampaignsListResponse getAdvertisersListResponse;
                    try
                    {
                        getAdvertisersListResponse = GetCampaignsListResponse.FromJson(responseContent);
                    }
                    catch (Exception ex)
                    {
        ...
        public GetCampaignsListResponse Create()
        {
            return null;
            //using (HttpClient client = new HttpClient())
            //{
            ...
            //        catch (Exception ex)
            //        {
            //            throw new NXSException("There is error in parsing data from ProData. Problem in ProData API.");
            //        }
